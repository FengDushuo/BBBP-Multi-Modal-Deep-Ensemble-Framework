...
def gpu_train_model(fps_file,input_file,output):
    # 加载描述符数据
    gpu_fps = np.load(fps_file, allow_pickle=True)

    # 检查数据一致性
    data = pd.read_csv(input_file, sep="\t")  # 包含 SMILES 和目标值
    if len(gpu_fps) != len(data):
        raise ValueError(f"Mismatch between feature count ({len(gpu_fps)}) and data rows ({len(data)})")
    
    # 将 GPU 特征添加到 DataFrame
    data["GPU_Features"] = list(gpu_fps)

    # 转换为 NumPy 数组
    X = np.array([np.array(fp) for fp in data["GPU_Features"] if fp is not None])  # 展平特征
...
gpu_train_model("gpu_features.npy","B3DB_classification.tsv","output_gpu/")