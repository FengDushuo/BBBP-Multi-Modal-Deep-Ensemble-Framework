...
# Calculate MSE for each model to use as weight (lower MSE means higher weight)
model_mse = {name: mean_squared_error(y_test, model.predict(test_features)) for name, model in algorithms.items()}

# 计算神经网络模型的 MSE，并添加到 model_mse 中
nn_preds = np.array(all_predictions['NeuralNetwork'])
nn_mse = mean_squared_error(actuals, nn_preds)
model_mse['NeuralNetwork'] = nn_mse

# Sort models based on their performance (lower MSE = better)
model_weights = {name: 1 / mse for name, mse in model_mse.items()}  # Weighting by inverse MSE
total_weight = sum(model_weights.values())
model_weights = {name: weight / total_weight for name, weight in model_weights.items()}

# Select top-performing algorithms (same as before)
...
# Assign weights to the predictions of top algorithms
weighted_predictions = np.zeros(stacked_features.shape[0])

# Apply weights to predictions
for i, (name, _) in enumerate(top_algorithms):
    weighted_predictions += model_weights[name] * stacked_features[:, i]

# Validate consistency in dimensions
assert len(weighted_predictions) == len(actuals), "Mismatch between weighted predictions and actual values!"

# Evaluate final weighted ensemble model
mse = mean_squared_error(actuals, weighted_predictions)
r2 = r2_score(actuals, weighted_predictions)
print(f"Final Weighted Ensemble Model - MSE: {mse:.4f}, R²: {r2:.4f}")

# Visualize results
plt.scatter(actuals, weighted_predictions, alpha=0.5)
plt.xlabel("Actual Values")
plt.ylabel("Predicted Values")
plt.title("Weighted Ensemble Model Predicted vs Actual")
plt.savefig("weighted_ensemble_model_predictions_maccs_opt.png")
plt.show()
