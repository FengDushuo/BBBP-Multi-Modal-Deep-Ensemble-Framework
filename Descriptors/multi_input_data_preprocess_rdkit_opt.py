...
output_path = 'D:/a_work/1-phD/project/5-VirtualScreening/processed_data_rdkit_opt.pkl'
...
invalid_smiles = data[data['rdkit'].apply(lambda x: x == [0] * 2048)]
log_message(f"Invalid SMILES count: {len(invalid_smiles)}")
data = data[data['rdkit'].apply(lambda x: x != [0] * 2048)]
...
data['rdkit_Normalized'] = data['rdkit'].apply(
