...
def plot_learning_curve(estimator, X, y, title, output_path, cv=5, scoring="f1", n_jobs=-1):
    ...
    plt.xlabel("Training Examples", fontname='Times New Roman', fontsize=14)
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"{output_path}/{title.replace(' ', '_')}_learning_curve.png")
    plt.close()

def train_and_evaluate_morgan(input_file, fingerprint_file, output):
    # 加载数据
...
def train_and_evaluate_maccs(input_file, fingerprint_file, output):
    # 加载数据
    fingerprints = np.load(fingerprint_file, allow_pickle=True)
    data = pd.read_csv(input_file, sep="\t")
...
def train_and_evaluate_rdkit(input_file, fingerprint_file, output):
    # 加载数据
    fingerprints = np.load(fingerprint_file, allow_pickle=True)
    data = pd.read_csv(input_file, sep="\t")
...
train_and_evaluate_morgan("B3DB_classification.tsv","morgan_fingerprints.npy","./output/")
train_and_evaluate_maccs("B3DB_classification.tsv","maccs_fingerprints.npy","./output_maccs/")
train_and_evaluate_rdkit("B3DB_classification.tsv","rdkit_fingerprints.npy","./output_rdkit/")
