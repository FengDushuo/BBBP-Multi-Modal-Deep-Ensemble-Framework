...
def do_bert_train(fps_file,input_file,output):
    ...
    # 保存 metrics 到 CSV
    metrics_file = os.path.join(output, "metrics.csv")
    with open(metrics_file, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=bert_metrics.keys())
        writer.writeheader()
        writer.writerow(bert_metrics)
    print(f"Metrics saved to {metrics_file}")

    print("Evaluation Metrics:", bert_metrics)
