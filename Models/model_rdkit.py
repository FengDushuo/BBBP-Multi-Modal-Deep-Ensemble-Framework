...
def morgan_train_model(fps_file,input_file,output):
    ...
    # maccs_fps = np.load(fps_file, allow_pickle=True)
    rdkit_fps = np.load(fps_file, allow_pickle=True)
    ...
    # 展平特征（以 Morgan 指纹为例）
    X = np.array([np.array(fp) for fp in data["RDKit_Fingerprint"]])  # 特征
...
morgan_train_model("rdkit_fingerprints.npy","B3DB_classification.tsv","./output_rdkit/")
